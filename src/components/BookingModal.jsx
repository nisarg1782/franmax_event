...
const BookingModal = ({ onClose, type = "stall" }) => {
  ...
  // Always use live website for WhatsApp share (Improved)
  const liveDomain = "https://franxpo.com";
  const shareType = type || "stall";
  const currentLink = `${liveDomain}?type=${shareType}`;
  const encodedLink = encodeURIComponent(
    `Book your stall at Franmax Expo 2025! Click here: ${currentLink}`
  );
  const whatsappLink = `https://wa.me/?text=${encodedLink}`;

  // Dev helper: Warn if accidentally using localhost
  useEffect(() => {
    if (window.location.hostname.includes("localhost") || window.location.hostname.match(/^192\.168\./)) {
      console.log(
        "%c[DEV WARNING]%c WhatsApp link always points to LIVE site:",
